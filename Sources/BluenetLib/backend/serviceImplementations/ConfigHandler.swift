...
public class ConfigHandler {
    ...
    public func setSwitchcraft(enabled: Bool) -> Promise<Void> {
        ...
        return _writeControlPacket(bleManager: self.bleManager, handle, ControlPacketsGenerator.getSwitchCraftPacket(enabled))
    }
    ...
    public func setSunTimes(sunriseSecondsSinceMidnight: UInt32, sunsetSecondsSinceMidnight: UInt32) -> Promise<Void> {
        if self.bleManager.connectionState(handle).connectionProtocolVersion != .v1 {
   ...
   public func _getConfig<T>(_ config : StateTypeV3, id: UInt16 = 0) -> Promise<T> {
    ...
    public func _getConfig<T>(_ requestPacket: [UInt8], id: UInt16 = 0) -> Promise<T> {
        ...
        return Promise<T> { seal in
            ...
            _writePacketWithReply(bleManager: self.bleManager, handle: handle, service: readParams.service, readCharacteristic: readParams.characteristic, writeCommand: writeCommand)
               .done{ resultPacket -> Void in
                    do {
                        let result : T = try getConfigPayloadFromResultPacket(self.bleManager, self.handle, resultPacket)
                        seal.fulfill(result)
                    }
                    catch let err {
                        seal.reject(err)
    ...
    func _getConfigReadParameters() -> BleParameters {
        ...
        if self.bleManager.connectionState(handle).operationMode == .setup {
