 ...
 *
 */
open class AdvertismentValidator {
    
    open var validated = false
    open var validatedReferenceId : String?
    open var cantDecrypt = false
    open var failedTime : Double = 0
    ...
    open func releaseLockOnDecryption() {
        self.cantDecrypt = false
        self.failedValidationCounter = 0
    }
    
    open func update(advertisement: Advertisement) {
        ...
        if (incomingOperationMode == CrownstoneMode.operation) {
            for keySet in self.settings.keySetList {
                ...
                if (self.validate(advertisement, referenceId: keySet.referenceId)) {
                    self.addValidMeasurement(referenceId: keySet.referenceId)
                    return
                }
            }
        }
    ...
    func advertisementIsValidated(referenceId : String? = nil) {
        self.validated = true
        self.cantDecrypt = false
    ...
    func addValidMeasurement(referenceId: String) {
        if (self.validationMap[referenceId] != nil) {
            ...
            if (self.validationMap[referenceId]!.validMeasurementCount >= self.validationMatchThreshold) {
                self.advertisementIsValidated(referenceId: referenceId)
            }
